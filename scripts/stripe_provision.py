...
class StripeWebhookHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        result = {"ok": True, "type": etype}

        if etype == "checkout.session.completed" and self.store:
            session = event.get("data", {}).get("object", {})
            customer_id = str(session.get("customer") or session.get("id", ""))
            email = str(session.get("customer_details", {}).get("email", ""))
            existing = self.store.find_by_customer(customer_id)
            api_key = existing or self.store.provision(customer_id, email=email)
            result["api_key"] = api_key
            result["customer_id"] = customer_id
            print(f"[stripe] provisioned key for {customer_id} ({email})")

        elif etype == "customer.subscription.deleted" and self.store:
            sub = event.get("data", {}).get("object", {})
            customer_id = str(sub.get("customer", ""))
            existing = self.store.find_by_customer(customer_id)
            if existing:
                self.store.revoke(existing)
