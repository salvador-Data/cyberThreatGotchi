# CTG Lab Playground — play with your stack
...
The playground menus let you **experiment** with CTG lab tools in a guided, professor-style session — without running the full autorun chain or surprise reboots.
...
|-----|-------|--------|----------------|
| 0–2 | Windows | Menu **1** Wireshark IDS DiagnoseOnly | tshark, Npcap, log paths ready |
| 2–4 | Windows | Menu **2** Shield Status | DDG VPN/DNS + host NIC; optional Kali SSH |
| 4–6 | Windows | Menu **7** Start Kali VM | VM up; note in-guest playground command |
| 6–8 | Kali | Menu **9** Lab dry status | What's installed vs missing |
| 8–10 | Kali | Menu **3** Scrambler demo | tor / http / auto modes + GUI path |
| 10–12 | Kali | Menu **4** SIEM dry-run | IDS tail sources without rotate prompts |
| 12–14 | Kali | Menu **6** + **8** | Suricata tail + Tor curl check |
| 14–15 | Either | Menu **0** Exit | Review logs under `Backups/logs/` |
...
| 4 | SIEM dry-run | Tail IDS/ClamAV logs only — no shield prompts |
...
| 9 | Lab dry status | Bootstrap, services, share — no reboot |
...
4. SMS (optional): `.env` with `TWILIO_*` and `CTG_ALERT_SMS_TO` — see [WIRESHARK_IDS_SMS.md](WIRESHARK_IDS_SMS.md)
...
- [CTG_LAB_AUTORUN.md](CTG_LAB_AUTORUN.md) — full stack orchestration
- [KALI_WIFI_ETH_PROMISC.md](KALI_WIFI_ETH_PROMISC.md) — promisc vs monitor
- [CTG_SHIELD_SIEM_PLAYBOOK.md](CTG_SHIELD_SIEM_PLAYBOOK.md) — live SIEM hook with y/n rotate
- [WIRESHARK_IDS_SMS.md](WIRESHARK_IDS_SMS.md) — Windows IDS + Twilio
