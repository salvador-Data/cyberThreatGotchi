...
**Hacker Planet LLC** — manual partner sourcing for curated lab hardware. Customer pays HPL via Stripe → operator buys from supplier/marketplace → ships to customer. **No** stored marketplace cards, **no** API auto-purchase.

Public language: **partner fulfillment** or **curated hardware** — never “dropship” on the website.

See also: [MARKET_PRICING_INTERNET.md](MARKET_PRICING_INTERNET.md) · [MARKET_PRICING_EBAY.md](MARKET_PRICING_EBAY.md) · [DROPSHIP_FULFILLMENT_RUNBOOK.md](DROPSHIP_FULFILLMENT_RUNBOOK.md)
...
3. **Operator dashboard** — `website/operator/fulfillment.html`:
   - **Copy ship-to** → paste into checkout
   - **Open supplier listing** → primary URL
   - **Search eBay / Amazon** → fallback search links when primary OOS
   - Save tracking URL + status when shipped

4. **Buy manually** — ship **direct to customer** unless forwarding from Philly.
...
5. **Channel-specific checks**
   ...
   - **Flipper / HackRF / O.MG cable:** reference pricing in docs only — not HPL catalog SKUs

6. **Margin gate** — if supplier listing exceeds retail minus Stripe/fees, pause and contact customer.
...
- Never scrape listing images into the shop — link out only.
...
- [EBAY_PARTNER_FULFILLMENT.md](EBAY_PARTNER_FULFILLMENT.md) — legacy eBay-focused note (superseded by this runbook)
