# Google Drive upload â€” Hacker Planet portfolio (andygkowal@gmail.com)
...
| Path | Contents |
|------|----------|
| `C:\Users\Owner\OneDrive\HackerPlanet-Portfolio\` | 4Ã— `.md` + 4Ã— `.html` portfolio exports |
...
## Option A â€” Upload via drive.google.com (recommended today)

1. Sign in at [https://drive.google.com](https://drive.google.com) as **andygkowal@gmail.com**.
2. Click **+ New** â†’ **Folder** â†’ name it `HackerPlanet-Portfolio`.
3. Open the folder â†’ **+ New** â†’ **File upload** (or drag-drop).
4. Upload everything from `C:\Users\Owner\OneDrive\HackerPlanet-Portfolio\` (all `.md` and `.html` files).
5. In the folder list, select the folder (or each file) â†’ **Share** (person icon).
6. Under **General access**, change from *Restricted* to **Anyone with the link**.
7. Role: **Viewer** (read-only).
8. Click **Copy link** â€” that is your public portfolio link. Do not paste the link into the git repo if you treat it as semi-private; share it on LinkedIn/resume as you prefer.

**Folder-level share (one link for all files):** Share the `HackerPlanet-Portfolio` folder once with â€œAnyone with the linkâ€ â†’ Viewer; nested files inherit access.

---

## Option B â€” Google Drive for Desktop (future automation)
...
6. Set sharing on the folder as in Option A steps 5â€“8.
...
Then set **Anyone with the link â†’ Viewer** in the Drive web UI (sync client does not set public ACLs by itself).

---

## Option C â€” rclone (if you configure later)

Do **not** store OAuth tokens or `rclone.conf` in this repository.

1. Install rclone and run `rclone config` â†’ remote name e.g. `gdrive`, type Google Drive, account andygkowal@gmail.com.
2. Upload:
...
4. For **folder-wide** â€œanyone readerâ€, use Google Drive web UI on the folder, or Google Drive API `permissions.create` with `type=anyone`, `role=reader` (requires OAuth outside the repo).
...
```powershell
cd C:\Users\Owner\Programs\Hacker Planet LLC\cyberThreatGotchi\docs
```
...
- Public â€œAnyone with the linkâ€ means **unlisted but accessible** to anyone who has the URL â€” appropriate for portfolio sharing, not for private keys or customer data.
...
| Step | Done? |
|------|-------|
| Files in `HackerPlanet-Portfolio` on Drive | â˜ |
| Folder shared: Anyone with link â†’ Viewer | â˜ |
| Link copied for LinkedIn/resume | â˜ |
| HTML copies uploaded (optional) | â˜ |

*Last automated staging: agent upload task â€” Drive API / Desktop sync unavailable on host.*
