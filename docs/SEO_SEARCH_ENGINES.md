...
4. **Automated apply** (after copying value from GSC dashboard):
   ```powershell
   $env:CF_API_TOKEN = "your_token"   # never commit this
   python scripts/seo_verification_dns.py --google-txt "google-site-verification=XXXXXXXX"
   ```
...
3. Verification options:
   - **DNS CNAME** (recommended on Cloudflare) — Bing shows **Host** (subdomain label) and **Target** (e.g. `verify.bing.com`):
     ```powershell
     python scripts/seo_verification_dns.py --bing-cname YOUR_HOST_LABEL verify.bing.com
     ```
...
```powershell
.\scripts\seo_go_live_checklist.ps1   # checks live robots.txt for DuckDuckBot
```
...
Run automated preflight: `.\scripts\seo_go_live_checklist.ps1`
